  ...
  <div>
    ...
    <section class="product-list dark-theme" v-else>
      <div
        ...
        :key="product.id"
        @mouseover="showAddToCart(product.id)"
        @mouseleave="hideAddToCart(product.id)"
      ...
      >
        ...
        <div>
          <img :src="product.image" alt="iPhone 13" class="product-image" />
        </div>
        ...
        <div>
          <a style="display: none" :id="product.id" class="add-to-cart" href="#"
...
import { GetProductsBycategorie, GetResources } from "../services/GetResources";
export default {
  ...
  async created() {
    ...
    if (this.$route.name === "Allproducts") {
      console.log("i am in all products route");
